...
from review import build_or_load_gen_model, EarlyStopping
...
def get_data(cons):
    ...
    if cons.ablation == "commit":
        feature_path = r"./codereview_tokens_data_commit.csv"
    else:
        feature_path = r"./codereview_tokens_data_function.csv"
    ...
    if cons.ablation == "function":
        ...
        train_data = data[data[partition] == "train"]
        test_data = data[data[partition] == "test"]
        valid_data = data[data[partition] == "val"]
    ...
    if cons.ablation == 'function':
        for label in labels:
            ...
            cur_output = label
            if cur_output == 'cvss2_AU':
                encoder = OneHotEncoder(sparse=False)
                cur_y_train = encoder.fit_transform(cur_y_train)
        ...
        for label in labels:
            ...
            if cur_output == 'cvss2_AU':
                encoder = OneHotEncoder(sparse=False)
                cur_y_val = encoder.fit_transform(cur_y_val)
        ...
        for label in labels:
            ...
            if cur_output == 'cvss2_AU':
                encoder = OneHotEncoder(sparse=False)
                cur_y_test = encoder.fit_transform(cur_y_test)
        ...
        for label in labels:
            ...
            if cur_output == 'cvss2_AV' or cur_output == 'cvss2_AU':
                encoder = OneHotEncoder(sparse=False)
                # 执行独热编码
                cur_y_train = encoder.fit_transform(cur_y_train)
...
def get_labels(batch, dev):
    ...
    true_AU = batch[6].argmax(axis=1).long().to(dev)
...
if __name__ == "__main__":
    ...
    trainData, validData, testData = get_data(cons)

    config, model, _ = build_or_load_gen_model(cons, VIB=cons.VIB)
    ...
    for epoch in range(50):
        ...
        model.train()
        if cons.ablation == 'function':
            PRED_C, LABEL_C, PRED_I, LABEL_I, PRED_A, LABEL_A, PRED_AV, LABEL_AV, PRED_AC, LABEL_AC, PRED_AU, LABEL_AU, PRED_severity, LABEL_severity = pre_label_init_function()
        ...
        for step, batch in enumerate(trainData):
            ...
            pre_C, pre_I, pre_A, pre_AV, pre_AC, pre_AU, pre_severity, loss, features = model(input_ids=input_ids,
        ...
        with torch.no_grad():
            for step, batch in enumerate(validData):
                ...
                input_ids = batch[0].to(dev)
                attention_masks = (input_ids != 1).int().to(dev)
                pre_C, pre_I, pre_A, pre_AV, pre_AC, pre_AU, pre_severity, loss, _ = model(input_ids=input_ids,
                                                                                           attention_mask=attention_masks,
                                                                                           labels=labels_dict, epoch=1)
